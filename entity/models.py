...
class BertForEntity(BertPreTrainedModel):
    ...
    def _get_span_embeddings(self, input_ids, spans, token_type_ids=None, attention_mask=None):
        # input_ids(16,273), sequence_output(16,273,768), pooled_output(16,768) is considered as a sentence representation
        ...
        # torch.Size([16, 652]) [[1,2,3,...,,0,0],...,[1,2,3,...,1,2,1],[652 dim]]
        spans_width = spans[:, :, 2].view(spans.size(0), -1)
        # torch.Size([16, 652, 150])
        spans_width_embedding = self.width_embedding(spans_width)

        # Concatenate embeddings of left/right points and the width embedding
        # torch.Size([16, 652, 1686]) 16 batch size, 652 span num, 768+768+150=1686 dim
    ...
    def forward(self, input_ids, spans, spans_mask, spans_ner_label=None, token_type_ids=None, attention_mask=None):
        """
        Args:
        input_ids: tokens_tensor torch.Size([16, 273]), 16 samples, 273 toks,
            [102,6170,...,103,0,0,...,0]
        attention_mask: attention_mask_tensor, torch.Size([16, 273]), 1 or 0
        spans: bert_spans_tensor, torch.Size([16, 652, 3]),
            16 samples, 652 spans, 3 dims (start_tok, end_tok, span_width),
            [[1,1,1],[1,2,2],[1,7,7],...,[29,29,1],...,[0,0,0]]
        spans_mask: spans_mask_tensor, torch.Size([16, 652]), 1 or 0
        spans_ner_label: spans_ner_label_tensor, torch.Size([16, 652])
            [5,0,0,...,0,2,0,...,0]
        """
        # torch.Size([16, 652, 1686])
        # 1686 = 768 start tok embedding + 768 end tok embedding + 150 width embedding
        ...
        for layer in self.ner_classifier:
            # torch.Size([16, 652, 1686]) -FeedForward-> torch.Size([16, 652, 150])
            # torch.Size([16, 652, 150]) -Linear-> torch.Size([16, 652, 7])
            hidden = layer(hidden)
        ...
        if spans_ner_label is not None:
            ...
            if attention_mask is not None:
                active_loss = spans_mask.view(-1) == 1  # torch.Size([10432])
                active_logits = logits.view(-1, logits.shape[-1])  # torch.Size([10432,7])
...
class EntityModel:
    def __init__(self, args, num_ner_labels):
        ...
        if args.use_albert:
            ...
            self.bert_model = AlbertForEntity.from_pretrained(
                bert_model_name, num_ner_labels=num_ner_labels, max_span_length=args.max_span_length, return_dict=False
            )
        else:
    ...
    def run_batch(self, samples_list, try_cuda=True, training=True):
        ...
        (
            tokens_tensor,  # torch.Size([16, 273]), 16 samples, max 273 toks, [102,6170,...,103,0,0,...,0]
            attention_mask_tensor,  # torch.Size([16, 273]), 1/0
            bert_spans_tensor,  # torch.Size([16, 652, 3]), 16 samples, max 652 spans, 3 dims (start_berttok, end_berttok, span_width), [[1,1,1],[1,2,2],[1,7,7],...,[29,29,1],...,[0,0,0]]
            spans_mask_tensor,  # torch.Size([16, 652]), 1/0
            spans_ner_label_tensor,  # torch.Size([16, 652]), [5,0,0,...,0,2,0,...,0]
            sentence_length,  # torch.Size([16]), [22,38,85,...,28]
